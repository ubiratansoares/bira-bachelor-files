...
import compiler.*;
import compiler.t3a.DataType;
import compiler.t3b.Attributes.Operator;
import compiler.t3b.Symbol.Category;
...
public class ParserImpl implements SyntacticAnalyzer {
		...
		while(optional(TokenType.KEYWORD_VAR)) {
			...
			if(localError == false) {
				for(Symbol ident : dcvAtt.getVariables()) {
					...
					try {
						symbols.declare(ident);
						ident.setAddress(out.getStackPointer());
						out.moveStackPointer(+1);
						out.append(Code.MALLOC, 1);
					} catch(SemanticException e) {
	...
	private void dc_p(Attributes att) throws CompilerException {
		...
		Token procId; Symbol proc; Attributes dcpAtt;
		...
		while(optional(TokenType.KEYWORD_PROCEDURE)) {
			...
			if(localError == false) {
				proc = new Symbol(att.getScope(), procId);
			} else {
				proc = symbols.createNullIdentifier();
				proc.setScope(att.getScope());
				out.cancel();
			}
			proc.setCategory(Category.PROCEDURE);
			try {
				symbols.declare(proc);
				out.moveStackPointer(+1);
				jump = out.append(Code.JMP);
				proc.setAddress(out.getNextInstructionAddress());
			} catch(SemanticException e) {
				errors.add(new SemanticError(procId, e.getMessage()));
				out.cancel();
			}
			dcpAtt.clear();
			dcpAtt.setScope(proc);
			...
			//geração de código (resolvendo valor pendente)
			if(jump != null) {
                            if(proc.getIndex() != null) {
				int npar = proc.getIndex();
				int nvar = symbols.getVarCount(proc);
				out.append(Code.FREE, npar + nvar);
				out.moveStackPointer(-(nvar + npar));
				out.append(Code.JPR);
				out.moveStackPointer(-1);
				jump.setArgument(out.getNextInstructionAddress());
                            }
			}
			...
			if(localError == false) {
				...
				for(int i = 0; i < parlist.size(); i++) {
					...
					try {
						...
						out.append(Code.BIND); npar++;
						param.setAddress(out.getStackPointer());
						out.moveStackPointer(+1);
					} catch(SemanticException e) {
						errors.add(new SemanticError(
				...
				switch(ident.getCategory()) {
					...
					case VARIABLE:
						...
						if((ident.getType() == DataType.INTEGER) &&
							...
							//gerar código
							out.append(Code.STR, ident.getAddress());
						}
						break;
				}
			} catch(SemanticException e) {
	...
	private void procedureCallSemanticCheck(Attributes att) {
		if(localError == false) {
			if(att.getScope().getCategory() != Category.PROGRAM) {
				errors.add(new SemanticError(att.getToken(),
						"Um procedimento não pode " +
						"chamar outro procedimento."));
				out.cancel();
			}
			...
			try {
				...
				switch(proc.getCategory()) {
					...
					case PROCEDURE:
						...
						out.append(Code.JPP, proc.getAddress());
						ret.setArgument(out.getNextInstructionAddress());
						break;
				}
			} catch(SemanticException e) {
				errors.add(new SemanticError(att.getToken(),
		...
		for(int i = 0; i < args.size(); i++) {
			try {
				...
				if(arg.getType() == param.getType()) {
					arg = symbols.getElement(arg);
					out.append(Code.LDP, arg.getAddress());
					out.moveStackPointer(+1);
				} else {
				...
				errors.add(new SemanticError(att.getToken(),
						"Variável ou parâmetro não encontrado."));
				out.cancel();
			}
		}
		return;
	...
	private void argumentDeclarationSemanticCheck(Attributes att) {
		if(localError == false) {
			Symbol var, arg;
			arg = new Symbol(att.getScope(), att.getToken());
			try {
	...
	private void cmd(Attributes att) throws CompilerException {
		...
		switch(token.getType()) {
			...
			case KEYWORD_WHILE: {
				...
				int condAddr = out.getNextInstructionAddress();
				...
				Instruction exitLoop = out.append(Code.JPF);
				...
				out.append(Code.JMP, condAddr);
				exitLoop.setArgument(out.getNextInstructionAddress());
			} break;
			...
			case KEYWORD_IF: {
				...
				Instruction doElse = out.append(Code.JPF);
				...
				Instruction endIf = out.append(Code.JMP);

				doElse.setArgument(out.getNextInstructionAddress());

				pfalsa(att);

				endIf.setArgument(out.getNextInstructionAddress());
			} break;
			...
			case KEYWORD_REPEAT: {
				...
				int loopAddr = out.getNextInstructionAddress();
				...
				switch(arg.getCategory()) {
					...
					case PARAMETER:
						if(type.equals(arg.getType())) {
							//gerar código
							switch(att.getToken().getType()) {
								case KEYWORD_READ: 
									out.append(Code.READ);
									out.append(Code.STR, arg.getAddress());
									break;
								case KEYWORD_WRITE:
									out.append(Code.LDV, arg.getAddress());
									out.append(Code.WRITE);
									break;
							}
						} else {
							sameType = false;
	...
	private void condicao(Attributes att) throws CompilerException {
		...
		if(localError == false) {
			//gerar código
			switch(operator) {
				case REL_EQ: out.append(Code.CPEQ); break;
				case REL_NE: out.append(Code.CPNE); break;
				case REL_GE: out.append(Code.CPGE); break;
				case REL_GT: out.append(Code.CPGT); break;
				case REL_LE: out.append(Code.CPLE); break;
				case REL_LT: out.append(Code.CPLT); break;
			}
		}
	...
	private void expressao(Attributes att) throws CompilerException {
		...
		while(inFirstSetOfOP_AD()) {
			...
			if(localError == false) {
				...
				switch(operator) {
					case ADDITION:    out.append(Code.ADD); break;
					case SUBTRACTION: out.append(Code.SUB); break;
				}
			}
		}
	...
	private void termo(Attributes att) throws CompilerException {
		...
		//gerar código para operação unária
		if(localError == false) {
			switch(unaryOperator) {
				case UNARY_PLUS: break;
				case UNARY_MINUS:
					out.append(Code.INV); break;
			}
		}
	...
	private void fator(Attributes att) throws CompilerException {
		...
		switch(token.getType()) {
				...
				try {
					...
					switch(ident.getCategory()) {
						...
						case VARIABLE:
							...
							out.append(Code.LDV, ident.getAddress());
							break;
					}
				} catch(SemanticException e) {
					errors.add(new SemanticError(token,
			...
			case CONST_INTEGER: {
				int value = Integer.valueOf(token.getString());
				token = lexer.next();
				att.setType(DataType.INTEGER);
				out.append(Code.LDC, value);
			} break;

			case CONST_REAL: {
				double value = Double.valueOf(token.getString());
				token = lexer.next();
				att.setType(DataType.REAL);
				out.append(Code.LDC, value);
			} break;
