...
void ballPaint3D(PBall ball, TFloat border) {
	...
	glMaterialfv(GL_FRONT,GL_AMBIENT,ball->color3D);
	glMaterialfv(GL_FRONT,GL_DIFFUSE,ball->color3D);
	glMaterialfv(GL_FRONT,GL_SPECULAR,p4ToGLfv(1.0,1.0,1.0,1.0,v));
	glMaterialf(GL_FRONT,GL_SHININESS,D_LIGHT_BALL_SHININESS);
	glPushMatrix();
		glTranslatef(ball->x + border,ball->y + border,D_TABLE_FLOOR_HEIGHT + ball->radius);
		glutSolidSphere(ball->radius,D_SPHERE_QUALITY,D_SPHERE_QUALITY);
	glPopMatrix();
	// shadow.
	glShadeModel(GL_FLAT);
	p4ToGLfv(0.0,0.1,0.0,0.0,v);
	glMaterialfv(GL_FRONT,GL_AMBIENT,v);
	glMaterialfv(GL_FRONT,GL_DIFFUSE,v);
	glMaterialfv(GL_FRONT,GL_SPECULAR,p4ToGLfv(0.0,0.0,0.0,0.0,v));
	glMaterialf (GL_FRONT,GL_SHININESS,0);
	drawCircle3Dxy(ball->radius,ball->x + border,ball->y + border,D_TABLE_FLOOR_HEIGHT + D_PLANE_DIFF);
	//glDisable(GL_DEPTH_TEST);

}

void ballPaint2D(PBall ball, TFloat border) {
	glColor4fv(ball->color2D);
//	glColor3f(ball->r,ball->g,ball->b);
	drawCircle2D(ball->radius,ball->x + border,ball->y + border);
...
//	New velocities in these axes (after collision): ed<=1,  for elastic collision ed=1.
	TFloat	vaP1 = va1 + (1.0 + D_BALL_ELASTIC_COEFFICIENT) * (va2 - va1) / (1.0 + b1->mass / b2->mass);
	TFloat	vaP2 = va2 + (1.0 + D_BALL_ELASTIC_COEFFICIENT) * (va1 - va2) / (1.0 + b2->mass / b1->mass);
...
//	Undo the projections.
	...
	if ( vx1 < 0 && vy1 < 0 )
		b1->angle = D_PI + b1->angle;
	else if ( vx1 < 0 )
		b1->angle = D_PI - b1->angle;
	else if ( vy1 < 0 )
		b1->angle = D_2_PI - b1->angle;

	if ( vx2 < 0 && vy2 < 0 )
		b2->angle = D_PI + b2->angle;
	else if ( vx2 < 0 )
		b2->angle = D_PI - b2->angle;
	else if ( vy2 < 0 )
		b2->angle = D_2_PI - b2->angle;
...
TFloat ballSpeedAfter(PBall ball, TFloat m, TFloat time) {
	TFloat	speed = ball->speed - m * D_G * time;
...
void ballSetColor2D(PBall ball, float r, float g, float b) {
	p4ToGLfv(r,g,b,1.0,ball->color2D);
}

void ballSetColor3D(PBall ball, float r, float g, float b) {
	p4ToGLfv(r,g,b,1.0,ball->color3D);
}
