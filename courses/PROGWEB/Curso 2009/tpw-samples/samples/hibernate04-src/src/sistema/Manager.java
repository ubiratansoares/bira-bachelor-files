...
public class Manager {
    ...
    private void findUser(Long userId) {
	    ...
	    User user01 = (User) session.createQuery("select u from User u where u.id = :uid").setParameter("uid", userId).uniqueResult(); // busca e faz detachment
    ...
    private void findContasFromUser(Long userId) {
	    ...
	    /* PODE SER ESSE OU O CODIGO ABAIXO
	     * */
	    /*
	    Object[] vetor = user01.getContas().toArray();

	    for (int i = 0; i < vetor.length; i++) {
	    	Conta c = (Conta) vetor[i];
