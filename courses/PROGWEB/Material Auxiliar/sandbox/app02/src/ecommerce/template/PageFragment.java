...
import static ecommerce.template.TemplateElement.Type.*;

/**
 * <p>Representa um fragmento de documento em formato de texto. Por 
 ...
 * 
 * <p>O fragemento deve ser carregado de um molde, e opcionalmente pode
 * substituir variáveis ou processar instruções simples. As instruções
 * aceitas por esta implementação são:
 * 
 * <dl>
 * <dt>HEAD {param}:{valor}</dt>
 * <dd>Sugere um valor para um parâmetro do cabeçalho HTTP (o usuário 
 * decidirá aceitá-lo ou não);</dd>
 * 
 * <dt>VAR {id}</dt>
 * <dd>Variável de substituição (vazio por padrão, propaga indentações);</dd>
 * 
 * <dt>URI {id}</dt>
 * <dd>URI de substituição (vazio por padrão), semelhante ao VAR, mas 
 * relativiza endereços com relação a uma base;</dd>
 * 
 * <dt>I18N {id}</dt>
 * <dd>Texto de substituição semelhante ao VAR, mas obtém o conteúdo
 * a partir de um recurso de internacionalização relativo à localização
 * atual do fragmento;</dd>
 * 
 * <dt>LOCALE</dt>
 * <dd>Localização atual do fragmento (alguns elementos HTML usam o
 * atributo "lang" para indicar a localização do conteúdo do elemento), 
 * na forma de código ISO;</dd>
 * 
 * <dt>OPTIONAL {id} ... END {id}</dt>
 * <dd>Fragmento opcional (oculto por padrão);</dd>
 * 
 * <dt>LOOP {id} ... END {id}</dt>
 * <dd>Fragmento repetitivo (as repetições podem ser diferentes, pois 
 * também podem conter instruções);</dd>
 * 
 * <dt>SELECT {id} OPTION {id1} ... OPTION {id2} ... OPTION {idn} ... 
 * END {id}"</dt>
 * <dd>Fragmentos opcionais, escolhe uma das opções ou oculta 
 * todas elas (oculta todas por padrão).</dd>
 * 
 * <p>Além disso, cada fragmento gerencia suas próprias permissões, e o
 ...
 * 
 * <p>Por fim, cada fragmento gerencia um conjunto de recursos 
...
public abstract class PageFragment {
	 ...
	 * processado; {@code false} caso contrário.
	 */
	protected boolean isAllowed(Set<String> groups) {
		return true;
	}
	...
	/**
	 * Define um valor de substituição. Este método invoca o método
	 * {@link #get()} sobre o fragmento especificado.
	 * 
	 * @param id
	 * O identificador usado no molde para a instrução VAR.
	 * 
	 * @param value
	 * O valor que substituirá todas as instâncias da instrução
	 * VAR com identificador correspondente no molde. O fragmento
	 * será processado e apenas o resultado atual será usado.
	 */
	protected void setValue(String id, PageFragment value) {
		if(vars.containsKey(id))
			vars.put(id, (value == null)? "": value.get());
	}
	
	/**
	 * Define um valor de substituição. Este método invoca o método
	 * {@link #get(java.lang.String)} sobre o fragmento especificado.
	 * 
	 * @param id
	 * O identificador usado no molde para a instrução VAR.
	 * 
	 ...
	 * 
	 * @param group
	 * As credenciais usadas para construir o subfragmento.
	 */
	protected void setValue(String id, PageFragment value, String group) {
		if(vars.containsKey(id))
			vars.put(id, (value == null)? "": value.get(group));
	}
	
	/**
	 * Define um valor de substituição. Este método invoca o método
	 * {@link #get(java.util.Set)} sobre o fragmento especificado.
	 * 
	 * @param id
	 * O identificador usado no molde para a instrução VAR.
	 * 
	 ...
	 * As credenciais usadas para construir o subfragmento.
	 */
	protected void setValue(String id, PageFragment value, Set<String> groups) {
		if(vars.containsKey(id))
			vars.put(id, (value == null)? "": value.get(groups));
	}
	...
	/**
	 * Obtém a localização que será usada para a construção do 
	 * fragmento, ou ao menos aquela sugerida se a página não
	 * utilizar internacionalização.
	 * 
	 * @return 
	 * A localização que será usada para construir o fragmento.
	 */
	public Locale getLocale() {
		return (bundle == null)? locale: bundle.getLocale();
	}
	...
	/**
	 * Define as sugestões de valores para o cabeçalho HTTP.
	 * 
	 * @param values
	 * Os valores sugeridos pelo fragmento.
	 */
	protected void setHeaders(Map<String, String> values) {
		headers.putAll(values);
	}
	
	/**
	 * Remove uma sugestão de valor para um cabeçalho HTTP.
	 * 
	 * @param key
	 * O campo HTTP cujo valor será removido.
	 */
	...
	/**
	 * Apaga todas as sugestões de cabeçalhos HTTP.
	 */
	...
	/**
	 * Obtém um valor para um header HTTP sugerido pelo fragmento.
	 * 
	 * @param field
	 * O campo cujo valor deve ser obtido.
	 * 
	 * @return
	...
	/**
	 * Permite que o conteúdo de VAR e de I18N seja escapado
	 * antes de ser inserido. A implementação padrão apenas
	 * retorna o conteúdo original (isto é, não escapa nada).
	 * 
	 * @param text
	 * O texto a ser escapado. Pode ser {@code null}.
	 * 
	 * @return
	 * O texto escapado.
	 */
	...
	private void process(TemplateElement e, StringBuilder buffer) {
		switch(e.getType()) {
			...
			case I18N: { //replace
				String text = bundle.getString(e.getValue());
			...
			case LOCALE: { //replace
				...
				if(!l.getLanguage().isEmpty()) {
					buffer.append(l.getLanguage());
					if(!l.getCountry().isEmpty())
						buffer.append("-" + l.getCountry());
					if(!l.getVariant().isEmpty())
						buffer.append("-" + l.getVariant());
				}
			} return;
	...
	private Map<String, java.net.URI> targets;
	 ...
	 */
	protected PageFragment(PageTemplate template, Locale locale) {
		this.headers = new HashMap<String, String>();
		this.options = new HashMap<String, String>();
		this.vars = new HashMap<String, String>();
		this.flags = new HashMap<String, Boolean>();
	 ...
	 * 
	 * @param bundle
	 * O nome básico do recurso de internacionalização.
	 */
	protected PageFragment(PageTemplate template, Locale locale, String bundle) {
		this.headers = new HashMap<String, String>();
		this.options = new HashMap<String, String>();
		this.vars = new HashMap<String, String>();
		this.flags = new HashMap<String, Boolean>();
