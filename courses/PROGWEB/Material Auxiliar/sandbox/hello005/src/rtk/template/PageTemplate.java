...
public class PageTemplate {
	...
	private List<TemplateElement> parseText(String text) {
		...
		while(m.find()) {
			...
			if(instructions.containsKey(m.group(1))) {
				...
				flat.add(new TemplateElement(t, m.group(2)));
			} else {
	...
	public PageTemplate() {
		...
		this.template = new BlockElement(OPTIONAL, 
				"ROOT", new ArrayList<TemplateElement>());
	...
	/**
	 * Cria um molde com os elementos especificados.
	 */
	public PageTemplate(List<TemplateElement> template) {
		this.prefix = this.getPrefix();
		this.suffix = this.getSuffix();
		this.pattern = this.getPattern();
		this.instructions = this.getInstructionMap();
